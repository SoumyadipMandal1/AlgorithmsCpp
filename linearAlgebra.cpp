...
std::vector<float> gaussianElimination_partialPivoting(vector2d<float> coefficient, std::vector<float> augmented)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        // Iterate through the numbers in the pivot column and the number in the pivot position
        // Find the number with the maximum absolute value
        // Swap the row where the number is located with the pivot row

        maximum = absolute(coefficient[i][i]);
        maximumAbsolutValue_row = i;

        // Iterating through the numbers in the column below the pivot
        for (int j = i + 1; j < size; j++)
        {
            absoluteValue = absolute(coefficient[j][i]);
            if (maximum < absoluteValue)
            {
                maximum = absoluteValue;
                maximumAbsolutValue_row = j;
            }
        }
